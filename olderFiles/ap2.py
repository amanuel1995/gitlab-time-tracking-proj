...
def pullProjects():
    ...
    responseObject = requests.get(
        proj_url, headers={"PRIVATE-TOKEN": config.theToken})
...
def pullUsers():
    ...
    responseObject = requests.get(
        users_url, headers={"PRIVATE-TOKEN": config.theToken})
...
def pullNotes(projID, issueIID):
    ...
    for eachNote in noteJsonRes:
        ...
        if (result[0] == 'added'):
            ...
            year, month, week, day, hr, minutes = getTimeInfo(result)

            #calcuate
            postiveTime = (postiveTime + (year*YEARSECONDS) + (month*MONTHSECONDS) + (
                week*WEEKSECONDS) + (day*DAYSECONDS) + (hr*HOURSECONDS) + (minutes*MINSECONDS))
        ...
        if(result[0] == 'subtracted'):
            ...
            negTime = (negTime +
                             ...
                             (day*DAYSECONDS) + (hr*HOURSECONDS) + (minutes*MINSECONDS))
        # else:
        #     pass
